...
const LANGUAGE_MAP: Record<string, LangConfig> = {
  ...
  java: {
    ...
    fileExt: '.java',
    buildCmd: (f) => [
      `cp ${f} /tmp/Solution.java && javac /tmp/Solution.java && java -cp /tmp Solution`,
    ],
  },
  go: {
...
const DANGEROUS_PATTERNS: { pattern: RegExp; description: string }[] = [
  ...
  {
    pattern: /:\(\)\{.*\|.*&\s*\};\s*:/,
    description: 'Bash fork bomb detected',
  },
  { pattern: /fork\s*\(/, description: 'fork() system call detected' },
  // Reverse shells
  { pattern: /\/dev\/tcp\//, description: 'TCP device access (reverse shell)' },
  {
    pattern: /socket\s*\.\s*connect/i,
    description: 'Socket connect (network access)',
  },
  {
    pattern: /net\.connect|net\.createConnection/,
    description: 'Node.js net module usage',
  },
  // Process/system escape — JavaScript / TypeScript
  {
    pattern: /child_process/,
    description: 'child_process module (sandbox escape)',
  },
  {
    pattern: /require\s*\(\s*['"]child_process/,
    description: 'Requiring child_process',
  },
  // Process/system escape — Python
  {
    pattern: /os\s*\.\s*system\s*\(/,
    description: 'os.system() call (Python)',
  },
  ...
  // Process/system escape — Java
  {
    ...
    description: 'Runtime.exec() (Java sandbox escape)',
  },
  {
    pattern: /ProcessBuilder/,
    description: 'ProcessBuilder (Java sandbox escape)',
  },
  ...
  // Process/system escape — Rust
  {
    ...
    description: 'std::process::Command (Rust sandbox escape)',
  },
  // Shell commands
  {
    pattern: /exec\s*\(\s*['"].*rm\s/,
    description: 'Shell exec with rm command',
  },
  // File system escape
...
export class SandboxService {
  ...
  constructor(private config: ConfigService) {
    ...
    const rawTimeout = this.config.get<string | number>(
      'SANDBOX_TIMEOUT_MS',
      10000,
    );
    ...
    this.logger.log(
      `Sandbox initialized: Timeout=${this.timeoutMs}ms, Memory=${this.memoryLimit}, CPU=${this.cpuLimit}`,
    );
  }
  ...
  async executeCode(
    ...
    const { dir, filePath, fileName } = await this.writeTempFile(
      code,
      langCfg.fileExt,
    );
  ...
  async evaluateAgainstTests(
    ...
    try {
      for (const test of tests) {
        const exec = await this.runContainer(
          langCfg,
          dir,
          fileName,
          test.input,
      ...
      execFile(
        ...
        (error, stdout, stderr) => {
          if (error || !stdout) {
            if (error) {
              this.logger.debug(
                `Docker stats failed for ${containerName}: ${error.message} ${stderr || ''}`,
              );
            }
            return resolve(null);
          }
  ...
  private runContainer(
    ...
    const args = [
      ...
      containerName,
      '-v',
      `${hostDir}/${fileName}:${containerCodePath}:ro`,
      '-i',
      langCfg.image,
      ...cmd,
    ];
    ...
    this.logger.debug(
      `Stdin: ${stdin ? JSON.stringify(stdin.substring(0, 200)) : '(empty)'}`,
    );

    return new Promise<ExecutionResult>((resolve) => {
      let maxMemMb = 0;
      let maxCpu = 0;
      const statsPromise = this.getDockerStats(containerName)
        .then((stats) => {
          if (!stats) return;
          ...
          const numericCode =
            typeof rawCode === 'number'
              ...
              : typeof rawCode === 'string'
                ? Number(rawCode)
                : 0;
          const exitCode = timedOut ? 124 : numericCode;

          this.logger.debug(
            `Docker result: exitCode=${exitCode}, timedOut=${timedOut}, timeMs=${timeMs}`,
          );
          this.logger.debug(
            `Docker stdout: ${(stdout || '').substring(0, 300) || '(empty)'}`,
          );
          if (stderr)
            this.logger.debug(
              `Docker stderr: ${(stderr || '').substring(0, 300)}`,
            );
      ...
      // Convert spaces and literal \n sequences to real newlines so that input() can read each value sequentially
      // Always write to stdin (even if empty) before calling end() to prevent EOFError
      if (stdin !== undefined && stdin !== null) {
        const normalizedStdin = stdin.replace(/ /g, '\n').replace(/\\n/g, '\n');
