/**
 * Seed — Canvas Challenges
 * Crée 6 challenges de type CANVAS (architecture logicielle)
 *
 * Usage :
 *   cd backend
 *   pnpm prisma:seed
...
const canvasChallenges = [
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    rubric: [
      { category: 'Clarté du diagramme', description: 'Légendes, flèches dirigées, composants nommés', maxPoints: 20 },
      { category: 'Scalabilité', description: 'Stratégie pour 1M+ utilisateurs simultanés', maxPoints: 25 },
      { category: 'Sécurité', description: 'Auth JWT, chiffrement TLS, rate limiting', maxPoints: 20 },
      { category: 'Résilience', description: 'Gestion des pannes, reconnexion, fallback', maxPoints: 20 },
      { category: 'Conformité RGPD', description: 'Chiffrement at-rest, droit à l\'oubli', maxPoints: 15 },
    ],
    constraints: {
      timeLimit: 45,
      maxElements: 50,
...
`,
  ...
  // ────────────────────────────────────────────────────
  // 2. Système de Notifications Push — Hard
  // ────────────────────────────────────────────────────
  {
    ...
    rubric: [
      { category: 'Architecture event-driven', description: 'Découplage producteur/consommateur', maxPoints: 25 },
      { category: 'Gestion des canaux', description: 'Email, SMS, push, in-app représentés', maxPoints: 20 },
      { category: 'Fiabilité', description: 'Retry, dead-letter queue, idempotence', maxPoints: 25 },
      { category: 'Personnalisation', description: 'Préférences utilisateur, quiet hours, fréquence', maxPoints: 15 },
      { category: 'Monitoring', description: 'Métriques de delivery, alertes, dashboard', maxPoints: 15 },
    ],
    constraints: {
      timeLimit: 60,
      maxElements: 60,
...
`,
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    rubric: [
      { category: 'Structure 3-tiers', description: 'Les 3 couches clairement séparées et nommées', maxPoints: 30 },
      { category: 'Composants essentiels', description: 'CDN, LB, Cache, DB présents', maxPoints: 30 },
      { category: 'Flux de requêtes', description: 'Flèches et sens du flux corrects', maxPoints: 25 },
      { category: 'Lisibilité', description: 'Diagramme propre et compréhensible', maxPoints: 15 },
    ],
    constraints: {
      timeLimit: 30,
      maxElements: 30,
...
`,
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    rubric: [
      { category: 'Pipeline CI', description: 'Étapes build, test, scan sécurité, push registry', maxPoints: 25 },
      { category: 'Stratégie CD', description: 'GitOps, blue/green ou canary deployment', maxPoints: 25 },
      { category: 'Kubernetes', description: 'Namespaces, HPA, ingress, secrets correctement représentés', maxPoints: 20 },
      { category: 'Sécurité DevSecOps', description: 'Scan vulnérabilités, secrets management, RBAC', maxPoints: 20 },
      { category: 'Observabilité', description: 'Logs, métriques, traces (Loki, Prometheus, Jaeger)', maxPoints: 10 },
    ],
    constraints: {
      timeLimit: 60,
      maxElements: 70,
      requiredComponents: ['Git Repo', 'CI Runner', 'Container Registry', 'K8s Cluster', 'Monitoring'],
...
`,
  ...
  // ────────────────────────────────────────────────────
  // 5. Architecture Microservices — Medium
  // ────────────────────────────────────────────────────
  {
    ...
    rubric: [
      { category: 'Découpage des services', description: 'Bounded contexts cohérents, responsabilités claires', maxPoints: 25 },
      { category: 'API Gateway', description: 'Auth, routing, rate limiting, circuit breaker', maxPoints: 20 },
      { category: 'Communication inter-services', description: 'Sync vs async bien choisi selon le cas d\'usage', maxPoints: 20 },
      { category: 'Isolation des données', description: 'Chaque service a sa propre DB', maxPoints: 20 },
      { category: 'Stratégie de migration', description: 'Strangler fig ou autre approche progressive', maxPoints: 15 },
    ],
    constraints: {
      timeLimit: 50,
      maxElements: 60,
...
`,
  ...
  // ────────────────────────────────────────────────────
  {
    ...
    rubric: [
      { category: 'Ingestion & Stockage', description: 'Sources de données, data lake, data warehouse', maxPoints: 20 },
      { category: 'Feature Engineering', description: 'Feature store, transformations, versioning', maxPoints: 20 },
      { category: 'Entraînement & Évaluation', description: 'Orchestration, métriques, model registry', maxPoints: 20 },
      { category: 'Déploiement', description: 'Serving API, A/B testing, rollout progressif', maxPoints: 20 },
      { category: 'Monitoring MLOps', description: 'Data drift, model decay, alertes, retraining auto', maxPoints: 20 },
    ],
    constraints: {
      timeLimit: 60,
      maxElements: 70,
      requiredComponents: ['Data Lake', 'Feature Store', 'Model Registry', 'Serving API', 'Monitoring'],
...
async function main() {
  console.log('🌱 Seeding Canvas Challenges...\n');

  let created = 0;
  let skipped = 0;

  for (const challenge of canvasChallenges) {
    const existing = await prisma.challenge.findFirst({
      where: { title: challenge.title, kind: 'CANVAS' },
    });
  ...
  console.log(`  📦 Total   : ${created + skipped} / ${canvasChallenges.length}`);

  // Vérification finale
  const total = await prisma.challenge.count({ where: { kind: 'CANVAS' } });
  console.log(`\n🎨 Challenges CANVAS en base : ${total}`);
}
