...
import { InterviewDomainEnum, InterviewLanguageEnum } from '../interviews/dto/interview.dto';
...
export class AiInterviewService {
    ...
    constructor(private configService: ConfigService) {
        ...
        if (!apiKey) {
            this.logger.warn('⚠️  GEMINI_API_KEY not configured - Interview AI disabled');
            return;
        }
    ...
    async generateResponse(params: {
        ...
        const conversationContext = this.buildConversationContext(params.conversationHistory, lang);
        const systemPrompt = this.buildSystemPrompt(domainConfig, params.difficulty, lang);
...
As an interviewer, provide an appropriate response, ask follow-up questions if relevant, and guide the candidate. Respond in English.`;
    ...
    async reviewCode(params: {
        ...
        const domainConfig = DOMAIN_PROMPTS[params.domain];
        const lang = params.interviewLanguage === InterviewLanguageEnum.EN ? 'en' : 'fr';
...
Be constructive and guide toward improvements. Respond in English.`;
    ...
    async generateFinalFeedback(params: {
        ...
        const evaluationCriteria = domainConfig.evaluationCriteria.map(c => `- ${c}`).join('\n');
...
Be warm, rigorous, and professional.`;
    ...
    private buildConversationContext(history: InterviewMessage[], lang: string): string {
    ...
    private parseFeedbackResponse(text: string, domainConfig: DomainPromptConfig, lang: string): InterviewFeedback {
        try {
            ...
            return {
                ...
                communicationScore: Math.min(10, Math.max(0, parsed.communicationScore || 5)),
                problemSolvingScore: Math.min(10, Math.max(0, parsed.problemSolvingScore || 5)),
                verdict: this.determineVerdict(parsed.overallScore || 5),
                competencyScores: Array.isArray(parsed.competencyScores)
                    ? parsed.competencyScores.map((c: { competency: string; score: number; feedback: string }) => ({
                ...
                recommendedResources: Array.isArray(parsed.recommendedResources)
                    ...
                    : [],
                closingMessage: parsed.closingMessage || (lang === 'fr'
    ...
    private getMockInitialPrompt(domain: InterviewDomainEnum, difficulty: string, lang: string): string {
        const domainConfig = DOMAIN_PROMPTS[domain];
...
Keep it up! 💪`;
    ...
    private getMockFeedback(domainConfig: DomainPromptConfig | undefined, lang: string): InterviewFeedback {
        ...
        if (lang === 'fr') {
            return {
                ...
                recommendedResources: [
                    { title: 'Introduction aux Algorithms', url: 'https://example.com/intro-algo', type: 'course' },
                    { title: 'Pratique du System Design', url: 'https://example.com/system-design', type: 'practice' }
                ],
            ...
            return {
                ...
                recommendedResources: [
                    { title: 'Introduction to Algorithms', url: 'https://example.com/intro-algo', type: 'course' },
                    { title: 'System Design Practice', url: 'https://example.com/system-design', type: 'practice' }
                ],
