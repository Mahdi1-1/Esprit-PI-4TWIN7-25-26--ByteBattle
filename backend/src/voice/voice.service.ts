...
export class VoiceService {
  ...
  private initializeClients() {
    ...
    if (credentialsPath && fs.existsSync(credentialsPath)) {
      try {
        ...
        this.logger.warn(
          "⚠️ Failed to initialize Google Cloud clients:",
          error,
        );
      }
    } else {
      this.logger.warn(
        "⚠️ Google Cloud credentials not found - Voice features disabled",
      );
    }
  }
  ...
  async textToSpeech(options: TTSOptions): Promise<TTSResult> {
    ...
    this.logger.log(
      `🔊 Converting text to speech (${text.length} chars, ${languageCode})`,
    );
  ...
  async speechToText(options: STTOptions): Promise<STTResult> {
    ...
    this.logger.log(
      `🎤 Converting speech to text (${audioBuffer.length} bytes, ${languageCode})`,
    );
    ...
    try {
      ...
      this.logger.log(
        `✅ STT completed: "${alternative.transcript?.substring(0, 50)}..."`,
      );
  ...
  async speechToTextFromMulterFile(
    audio: Express.Multer.File,
    languageCode: string,
  ): Promise<string> {
    this.logger.debug(
      `STT request: size=${audio.size}bytes, lang=${languageCode}`,
    );
    this.logger.debug(
    ...
    if (!this.sttClient) {
      throw new Error(
        "Google Speech-to-Text client is not initialized. Check GOOGLE_APPLICATION_CREDENTIALS.",
      );
    }
    ...
    this.logger.debug(
      `Sending to Google STT: ${audioBytes.length} base64 chars`,
    );
    ...
    this.logger.debug(
      `Google STT raw response: ${JSON.stringify(response.results)}`,
    );
