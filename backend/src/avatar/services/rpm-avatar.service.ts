...
export class RpmAvatarService implements OnModuleInit {
  ...
  private buildRenderUrl(avatarId: string, scene: string, expression: string): string {
  ...
  private async downloadAndProcessRender(avatarId: string, userId: string, scene: string, expression: string) {
    ...
    try {
      ...
      this.logger.error(`Failed to process avatar for user ${userId}: ${error.message}`);
  ...
  async saveAvatar(userId: string, glbUrl: string, scene = 'halfbody-portrait-v1', expression = 'neutral') {
    ...
    try {
        const result = await this.downloadAndProcessRender(avatarId, userId, scene, expression);
        ...
        this.logger.error('Error processing avatar image, falling back to basic details', e);
        localImageUrl = this.buildRenderUrl(avatarId, scene, expression);
        thumbnailUrl = localImageUrl;
    }
  ...
  async updateExpression(userId: string, expression: string) {
    ...
    return this.reprocessAvatar(userId, user.avatar.rpmModelUrl, user.avatar.scene, expression);
  }
  ...
  async updateScene(userId: string, scene: string) {
    ...
    return this.reprocessAvatar(userId, user.avatar.rpmModelUrl, scene, user.avatar.expression || 'neutral');
  }
  ...
  private async reprocessAvatar(userId: string, glbUrl: string, scene: string, expression: string) {
     const avatarId = this.extractAvatarId(glbUrl);
    
    // Don't delete immediately in case of failure
    const { localImageUrl, thumbnailUrl } = await this.downloadAndProcessRender(avatarId, userId, scene, expression);
    const renderUrl = this.buildRenderUrl(avatarId, scene, expression);
