...
export class IntelligenceService {
  ...
  private async post<T>(path: string, body: unknown): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new ServiceUnavailableException(
        `Intelligence engine ${path} failed (${response.status}): ${text}`,
      );
    }
  ...
  async getSubmitDecision(
    payload: IntelligenceSubmitRequest,
  ): Promise<IntelligenceSubmitResponse> {
    this.logger.debug(
      `Submitting telemetry to intelligence engine for challenge ${payload.challenge_id}`,
    );
    return this.post<IntelligenceSubmitResponse>("/submit", payload);
  }

  async getHintPolicy(
    payload: IntelligenceSubmitRequest & { force_level?: number },
  ): Promise<IntelligenceHintResponse> {
    this.logger.debug(
      `Requesting hint policy from intelligence engine for challenge ${payload.challenge_id}`,
    );
  ...
  async getProfile(payload: IntelligenceProfileRequest): Promise<any> {
    this.logger.debug(
      `Requesting skill profile from intelligence engine for user ${payload.user_id}`,
    );
