...
const mockSubmissionService = { submit: jest.fn(), runCode: jest.fn(), getSubmissions: jest.fn() };
const mockScoreboardService = { getScoreboard: jest.fn() };
const mockChatService = { sendMessage: jest.fn(), getMessages: jest.fn() };
const mockClarificationService = { createClarification: jest.fn(), answerClarification: jest.fn(), getClarifications: jest.fn() };
const mockAnnouncementService = { createAnnouncement: jest.fn(), getAnnouncements: jest.fn(), togglePin: jest.fn() };
...
describe('HackathonsController', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      ...
      providers: [
        ...
        { provide: HackathonSubmissionService, useValue: mockSubmissionService },
        { provide: HackathonScoreboardService, useValue: mockScoreboardService },
        { provide: HackathonChatService, useValue: mockChatService },
        { provide: HackathonClarificationService, useValue: mockClarificationService },
        { provide: HackathonAnnouncementService, useValue: mockAnnouncementService },
        { provide: HackathonMonitoringService, useValue: mockMonitoringService },
        { provide: HackathonAuditService, useValue: mockAuditService },
        { provide: HackathonPlagiarismService, useValue: mockPlagiarismService },
      ],
    }).compile();
  ...
  describe('findAll()', () => {
    it('should call hackathonsService.findAll with pagination and filters', async () => {
      ...
      expect(mockHackathonsService.findAll).toHaveBeenCalledWith(
        expect.objectContaining({ page: 1, limit: 10, status: 'active', scope: 'public' }),
      );
    });
  });
  ...
  describe('findOne()', () => {
    it('should call hackathonsService.findOne with id', async () => {
      mockHackathonsService.findOne.mockResolvedValue({ id: 'hack-1', title: 'ByteBattle Hack' });
  ...
  describe('create()', () => {
    it('should call hackathonsService.create', async () => {
      const dto = { title: 'New Hack', description: 'Test', startTime: new Date(), endTime: new Date() };
  ...
  describe('getScoreboard()', () => {
    it('should call scoreboardService.getScoreboard', async () => {
      mockScoreboardService.getScoreboard.mockResolvedValue([{ rank: 1, teamName: 'A', score: 100 }]);

      const result = await controller.getScoreboard('hack-1');

      expect(mockScoreboardService.getScoreboard).toHaveBeenCalledWith('hack-1', false);
  ...
  describe('sendChatMessage()', () => {
    it('should call chatService.sendMessage', async () => {
      ...
      mockChatService.sendMessage.mockResolvedValue({ id: 'msg-1', content: 'Hello team!' });

      const result = await controller.sendTeamMessage('hack-1', 'team-1', 'user-1', dto as any);

      expect(mockChatService.sendMessage).toHaveBeenCalledWith('hack-1', 'team-1', 'user-1', dto);
  ...
  describe('getChatMessages()', () => {
    it('should call chatService.getMessages with pagination options', async () => {
      ...
      expect(mockChatService.getMessages).toHaveBeenCalledWith('hack-1', 'team-1', { before: undefined, limit: 50 });
    });
  });

  describe('getMonitoring()', () => {
    it('should call monitoringService.getMonitoringData', async () => {
      mockMonitoringService.getMonitoringData.mockResolvedValue({ totalSubmissions: 50 });

      const result = await controller.getMonitoring('hack-1');

      expect(mockMonitoringService.getMonitoringData).toHaveBeenCalledWith('hack-1');
  ...
  describe('checkPlagiarism()', () => {
    it('should call plagiarismService.checkPlagiarism', async () => {
      mockPlagiarismService.checkPlagiarism.mockResolvedValue({ flaggedPairs: [], totalCompared: 5 });
  ...
  describe('createAnnouncement()', () => {
    it('should call announcementService.createAnnouncement', async () => {
      ...
      mockAnnouncementService.createAnnouncement.mockResolvedValue({ id: 'ann-1' });
      ...
      expect(mockAnnouncementService.createAnnouncement).toHaveBeenCalledWith(
        'hack-1', 'admin-1', dto.content, dto.isPinned,
      );
    });
  });
});
