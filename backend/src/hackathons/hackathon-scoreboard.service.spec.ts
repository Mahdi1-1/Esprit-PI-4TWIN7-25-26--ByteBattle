...
describe('HackathonScoreboardService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<HackathonScoreboardService>(HackathonScoreboardService);
  ...
  it('should return frozen scoreboard from Redis for non-admin', async () => {
    const frozenBoard: Scoreboard = {
      ...
      status: 'frozen',
      challengeIds: ['c1'],
      rows: [{ rank: 1, teamId: 't1', teamName: 'T1', members: [], solved: 1, penalty: 30, problems: {} }],
      isFrozen: true,
      generatedAt: new Date().toISOString(),
    };

    prismaMock.hackathon.findUnique.mockResolvedValue({ ...HACKATHON, status: 'frozen' });
