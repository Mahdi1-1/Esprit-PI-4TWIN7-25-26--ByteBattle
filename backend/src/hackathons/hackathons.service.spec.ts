...
describe('HackathonsService — Lifecycle State Machine', () => {
  ...
  describe('Valid transitions', () => {
    ...
    for (const { from, to } of validTransitions) {
      it(`should allow transition from '${from}' to '${to}'`, async () => {
        ...
        prismaMock.hackathon.update.mockResolvedValue({ ...hackathon, status: to });
  ...
  describe('cancelHackathon', () => {
    it('should hard-delete draft hackathons', async () => {
      prismaMock.hackathon.findUnique.mockResolvedValue(makeHackathon({ status: 'draft' }));
      prismaMock.hackathon.delete.mockResolvedValue(undefined);

      const result = await service.cancelHackathon('h1', 'admin1', 'No longer needed');
      expect(result).toEqual({ deleted: true });
      expect(prismaMock.hackathon.delete).toHaveBeenCalledWith({ where: { id: 'h1' } });
    });

    it('should soft-cancel lobby hackathons', async () => {
      const hackathon = makeHackathon({ status: 'lobby' });
      prismaMock.hackathon.findUnique.mockResolvedValue(hackathon);
      prismaMock.hackathon.update.mockResolvedValue({ ...hackathon, status: 'cancelled' });
      ...
      expect(auditMock.log).toHaveBeenCalledWith('h1', 'admin1', 'cancel', { reason: 'Cancelled' });
    });

    it('should reject cancellation of active hackathons', async () => {
      prismaMock.hackathon.findUnique.mockResolvedValue(makeHackathon({ status: 'active' }));
  ...
  describe('deleteHackathon', () => {
    ...
    it('should forbid delete of active hackathon', async () => {
      ...
      await expect(
        service.deleteHackathon('h1', 'admin1'),
      ).rejects.toThrow(ForbiddenException);
    });
    ...
    it('should archive ended hackathon on delete', async () => {
      ...
      prismaMock.hackathon.findUnique.mockResolvedValue(hackathon);
      prismaMock.hackathon.update.mockResolvedValue({ ...hackathon, status: 'archived' });
  ...
  describe('update (edit guards)', () => {
    it('should allow edits in draft status', async () => {
      ...
      prismaMock.hackathon.findUnique.mockResolvedValue(hackathon);
      prismaMock.hackathon.update.mockResolvedValue({ ...hackathon, title: 'Updated' });
