...
describe("DiscussionsService", () => {
  ...
  describe("createDiscussion()", () => {
    it("should create a discussion and trigger badge check", async () => {
      ...
      expect(mockBadgeEngine.onDiscussionCreated).toHaveBeenCalledWith(
        "user-1",
      );
  ...
  describe("findOneDiscussion()", () => {
    it("should throw NotFoundException when discussion not found", async () => {
      ...
      await expect(service.findOneDiscussion("nonexistent")).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe("updateDiscussion()", () => {
    it("should throw NotFoundException when discussion not found", async () => {
      mockPrisma.discussion.findUnique.mockResolvedValue(null);
      await expect(
        service.updateDiscussion("disc-1", "user-1", { title: "New" } as any),
    ...
    it("should throw ForbiddenException when non-owner tries to update", async () => {
      mockPrisma.discussion.findUnique.mockResolvedValue(mockDiscussion);
      await expect(
        service.updateDiscussion("disc-1", "user-2", { title: "X" } as any),
    ...
    it("should allow owner to update", async () => {
      ...
      const result = await service.updateDiscussion("disc-1", "user-1", {
        title: "Updated",
      } as any);
  ...
  describe("deleteDiscussion()", () => {
    it("should throw ForbiddenException when non-owner tries to delete", async () => {
      mockPrisma.discussion.findUnique.mockResolvedValue(mockDiscussion);
      await expect(
        service.deleteDiscussion("disc-1", "user-9", "user"),
      ).rejects.toThrow(ForbiddenException);
    });
  ...
  describe("createComment()", () => {
    it("should throw NotFoundException when discussion not found", async () => {
      mockPrisma.discussion.findUnique.mockResolvedValue(null);
      await expect(
        service.createComment("user-1", "disc-1", { content: "Nice!" }),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it("should create comment and trigger badge + notification", async () => {
      ...
      const result = await service.createComment("user-2", "disc-1", {
        content: "Nice!",
      });
