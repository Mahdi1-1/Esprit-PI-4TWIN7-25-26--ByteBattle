...
export class DuelsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleJoinDuel(@MessageBody() data: { duelId: string }, @ConnectedSocket() client: Socket) {
    const userId = this.clientMaps.get(client.id);
    if (!userId) return;
  ...
  async handlePlayerReady(@MessageBody() data: { duelId: string }, @ConnectedSocket() client: Socket) {
    const userId = this.clientMaps.get(client.id);
    if (!userId) return;
