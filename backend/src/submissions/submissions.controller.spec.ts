...
describe('SubmissionsController', () => {
  ...
  describe('drafts()', () => {
    it('should save draft', async () => {
      ...
      const result = await controller.saveDraft('user-1', { challengeId: 'c1' } as any);
      expect(mockSubmissionsService.saveDraft).toHaveBeenCalledWith('user-1', { challengeId: 'c1' });
    ...
    it('should get draft', async () => {
      ...
      expect(mockSubmissionsService.findDraft).toHaveBeenCalledWith('user-1', 'c1');
    });

    it('should delete draft', async () => {
      mockSubmissionsService.deleteDraft.mockResolvedValue({ deleted: 1 });
      const result = await controller.deleteDraft('user-1', 'c1');
      expect(mockSubmissionsService.deleteDraft).toHaveBeenCalledWith('user-1', 'c1');
  ...
  describe('getStatus()', () => {
    it('should map status from submission', async () => {
      ...
      const result = await controller.getStatus('sub-1', { id: 'user-1', role: 'user' });
  ...
  describe('generateAiReview()', () => {
    it('should delegate to submissionsService', async () => {
      mockSubmissionsService.generateAiReview.mockResolvedValue({ id: 'review-1' });

      const result = await controller.generateAiReview('sub-1');

      expect(mockSubmissionsService.generateAiReview).toHaveBeenCalledWith('sub-1');
