...
export class AdminService {
  ...
  async changeUserRole(actorId: string, targetId: string, role: AllowedRole) {
    ...
    // Audit trail
    await this.createAuditLog(actorId, "USER_ROLE_CHANGED", "User", targetId, {
      previousRole,
      newRole: role,
  ...
  async getDashboardStats() {
    ...
    const [
      ...
      this.prisma.user
        .count({ where: { createdAt: { gte: h24 } } })
        .catch(() => 0),
      this.prisma.user
        .count({ where: { createdAt: { gte: prevH24, lt: h24 } } })
        .catch(() => 0),

      // Users by role/premium
      this.prisma.user.count({ where: { isPremium: true } }).catch(() => 0),
      this.prisma.user
        .count({ where: { role: "admin" as any } })
        .catch(() => 0),
      this.prisma.user
        .count({ where: { role: "moderator" as any } })
        .catch(() => 0),
      ...
      this.prisma.submission
        ...
        .catch(() => 0),
      this.prisma.submission
        .count({ where: { createdAt: { gte: d7 } } })
        .catch(() => 0),
      this.prisma.submission
        .count({ where: { createdAt: { gte: prevD7, lt: d7 } } })
        .catch(() => 0),
      this.prisma.submission.count({ where: { verdict: "AC" } }).catch(() => 0),

      // Hackathons
      this.prisma.hackathon.count().catch(() => 0),
      this.prisma.hackathon
        .count({ where: { status: { in: ["active", "lobby", "checkin"] } } })
        .catch(() => 0),
    ...
    const verdictTotal =
      ...
      verdictQueued;
    const pct = (v: number) =>
      verdictTotal > 0 ? Math.round((v / verdictTotal) * 1000) / 10 : 0;
  ...
  async getTimeSeries(days: number = 30) {
    ...
    const [users, submissions, duels, hackathonSubs] = await Promise.all([
      ...
      this.prisma.duel
        ...
        .catch(() => []),
      this.prisma.hackathonSubmission
        .findMany({
    ...
    for (const d of duels) {
      ...
      duelMap[k] = (duelMap[k] || 0) + 1;
    }
    for (const h of hackathonSubs) {
      const k = dateKey(h.submittedAt);
      hackMap[k] = (hackMap[k] || 0) + 1;
    }
  ...
  async getRetentionMetrics() {
    ...
    const [
      ...
      this.prisma.submission
        .groupBy({ by: ["userId"], where: { createdAt: { gte: h24 } } })
        .then((r) => r.length)
        .catch(() => 0),
      // WAU
      this.prisma.submission
        .groupBy({ by: ["userId"], where: { createdAt: { gte: d7 } } })
        .then((r) => r.length)
        .catch(() => 0),
      // MAU
      this.prisma.submission
        .groupBy({ by: ["userId"], where: { createdAt: { gte: d30 } } })
        .then((r) => r.length)
        .catch(() => 0),
      // Active submitters
      this.prisma.submission
      ...
      this.prisma.submission
        ...
        .then(async (rows) => {
          ...
          return rows.map((r) => ({
            ...
            elo: users.find((u) => u.id === r.userId)?.elo || 0,
          }));
        })
        .catch(() => []),
      ...
      this.prisma.duel
        ...
        .then(async (rows) => {
          ...
          return rows.map((r) => ({
            userId: r.player1Id,
            count: r._count.id,
            username: users.find((u) => u.id === r.player1Id)?.username || "?",
            elo: users.find((u) => u.id === r.player1Id)?.elo || 0,
          }));
        })
        .catch(() => []),
      // New users cohorts
    ...
    return {
      ...
      topDuelPlayers: (topDuelPlayers as any[]).map((r) => ({
        username: r.username,
        count: r.count,
        elo: r.elo,
      })),
      ...
      this.prisma.submission
        ...
        .then(async (rows) => {
          ...
          const acCounts = await Promise.all(
            ids.map((id) =>
              this.prisma.submission
                .count({ where: { challengeId: id, verdict: "AC" } })
                .catch(() => 0),
            ),
          );
          return rows.map((r, i) => {
            ...
            return {
              ...
              acceptRate:
                r._count.id > 0
                  ? Math.round((acCounts[i] / r._count.id) * 100)
                  : 0,
            };
          });
        })
  ...
  async getModuleUsage() {
    ...
    const totalActiveUsers = await this.prisma.user
    ...
    const [
      ...
      this.prisma.submission
        .count({ where: { kind: "CODE", createdAt: { gte: d7 } } })
        .catch(() => 0),
      this.prisma.submission
        .count({ where: { kind: "CODE", createdAt: { gte: prev7, lt: d7 } } })
        .catch(() => 0),
      this.prisma.submission
        .groupBy({
          by: ["userId"],
          where: { kind: "CODE", createdAt: { gte: d7 } },
        })
        .then((r) => r.length)
        .catch(() => 0),
      this.prisma.submission
        .aggregate({
          _avg: { timeMs: true },
          where: { kind: "CODE", verdict: "AC", timeMs: { not: null } },
        })
        .catch(() => ({ _avg: { timeMs: null } })),
      this.prisma.submission
        .findMany({
          where: { kind: "CODE", code: { not: null } },
          select: { code: true },
          take: 200,
        })
        .then((rows) =>
          rows.length
            ? Math.round(
                rows.reduce((s, r) => s + (r.code?.length ?? 0), 0) /
                  rows.length,
              )
            : 0,
        )
      ...
      // Canvas
      this.prisma.submission
        .count({ where: { kind: "CANVAS" } })
        .catch(() => 0),
      this.prisma.submission
        .count({ where: { kind: "CANVAS", createdAt: { gte: d7 } } })
        .catch(() => 0),
      this.prisma.submission
        .count({ where: { kind: "CANVAS", createdAt: { gte: prev7, lt: d7 } } })
        .catch(() => 0),
      this.prisma.submission
        .groupBy({
          by: ["userId"],
          where: { kind: "CANVAS", createdAt: { gte: d7 } },
        })
        .then((r) => r.length)
        .catch(() => 0),
      ...
      this.prisma.duel
        ...
        .catch(() => 0),
      this.prisma.duel
        .count({ where: { createdAt: { gte: prev7, lt: d7 } } })
        .catch(() => 0),
      this.prisma.duel
        .groupBy({ by: ["player1Id"], where: { createdAt: { gte: d7 } } })
        .then((r) => r.length)
        .catch(() => 0),
      this.prisma.duel
        .findMany({
      ...
      this.prisma.hackathonSubmission.count().catch(() => 0),
      this.prisma.hackathonSubmission
        .count({ where: { submittedAt: { gte: d7 } } })
        .catch(() => 0),
      this.prisma.hackathonSubmission
        .count({ where: { submittedAt: { gte: prev7, lt: d7 } } })
        .catch(() => 0),
      this.prisma.hackathonSubmission
        .groupBy({ by: ["userId"], where: { submittedAt: { gte: d7 } } })
        .then((r) => r.length)
        .catch(() => 0),
      this.prisma.hackathonTeam
        .findMany({ select: { members: true }, take: 200 })
        .then((teams) =>
          teams.length
            ? Math.round(
      ...
      this.prisma.interviewSession
        ...
        .catch(() => 0),
      this.prisma.interviewSession
        .findMany({ select: { messages: true }, take: 200 })
        .then((sessions) =>
          sessions.length
            ? Math.round(
                (sessions.reduce((s, sess) => s + sess.messages.length, 0) /
                  sessions.length) *
                  10,
              ) / 10
            : 0,
      ...
      this.prisma.discussion
        ...
        .catch(() => 0),
      this.prisma.discussion
        .groupBy({ by: ["authorId"], where: { createdAt: { gte: d7 } } })
        .then((r) => r.length)
        .catch(() => 0),
      this.prisma.comment
        .count({ where: { createdAt: { gte: d7 } } })
      ...
      this.prisma.aIReview
        ...
        .catch(() => 0),
      this.prisma.aIReview
        .groupBy({ by: ["submissionId"], where: { createdAt: { gte: d7 } } })
        .then((r) => r.length)
        .catch(() => 0),
      this.prisma.aIReview
        .aggregate({ _avg: { score: true }, where: { status: "completed" } })
      ...
      this.prisma.notification
        ...
        .catch(() => 0),
      this.prisma.notification
        .count({ where: { isRead: true, createdAt: { gte: d7 } } })
        .catch(() => 0),
    ]);

    const duelCompletion =
      totalDuels > 0 ? Math.round((duelCompletionRaw / totalDuels) * 100) : 0;
    const notifReadRate =
      notifs7d > 0 ? Math.round((readNotifs7d / notifs7d) * 100) : 0;
      ...
      modules: [
        {
          ...
          metrics: [
            {
              label: "Avg Exec Time",
              value: `${Math.round(avgExecTime._avg.timeMs ?? 0)} ms`,
              raw: avgExecTime._avg.timeMs ?? 0,
            },
            {
              label: "Avg Code Length",
              value: `${avgCodeLen} chars`,
              raw: avgCodeLen,
            },
          ],
        },
        ...
        },
        {
          ...
          metrics: [
            {
              label: "Avg Duration",
              value: `${avgDuelDurationRaw} min`,
              raw: avgDuelDurationRaw,
            },
            {
              label: "Completion Rate",
              value: `${duelCompletion}%`,
              raw: duelCompletion,
            },
          ],
        },
          ...
          metrics: [
            {
              ...
              value: `${avgInterviewMessages}`,
              raw: avgInterviewMessages,
            },
          ],
        },
        {
        ...
        },
        {
          ...
          metrics: [
            {
              label: "Avg Score",
              value: `${Math.round(avgReviewScore._avg?.score ?? 0)}/100`,
              raw: avgReviewScore._avg?.score ?? 0,
            },
          ],
        },
        {
