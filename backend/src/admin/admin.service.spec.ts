...
const mockPrisma = {
  user: { findUnique: jest.fn(), update: jest.fn(), count: jest.fn(), aggregate: jest.fn() },
...
describe('AdminService', () => {
  ...
  describe('changeUserRole()', () => {
    ...
    it('should update role and create audit log', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-2', username: 'bob', role: 'user' });
      mockPrisma.user.update.mockResolvedValue({ id: 'user-2', username: 'bob', role: 'moderator' });
      mockPrisma.auditLog.create.mockResolvedValue({});

      const result = await service.changeUserRole('admin-1', 'user-2', 'moderator');
  ...
  describe('updateReportStatus()', () => {
    it('should throw NotFoundException when report not found', async () => {
      ...
      await expect(service.updateReportStatus('rep-1', { status: 'resolved' })).rejects.toThrow(
        NotFoundException,
      );
    });

    it('should update report status', async () => {
      mockPrisma.report.findUnique.mockResolvedValue({ id: 'rep-1', status: 'open' });
      mockPrisma.report.update.mockResolvedValue({ id: 'rep-1', status: 'resolved' });

      const result = await service.updateReportStatus('rep-1', { status: 'resolved' });
  ...
  describe('getDashboardStats()', () => {
    it('should return stats object without crashing', async () => {
      ...
      mockPrisma.user.aggregate.mockResolvedValue({ _avg: { level: null, xp: null, elo: null } });
      mockPrisma.report.count.mockResolvedValue(0);
