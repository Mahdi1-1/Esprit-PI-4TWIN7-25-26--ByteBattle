...
import {
  computeDuelStats,
  computeDuelStatsBatch,
} from "../duels/duel-stats.util";
...
export class LeaderboardService {
  ...
  async getGlobal(query: {
    ...
    if (query.language) {
      ...
      const ranked = processedUsers.map((u, i) => ({
        ...u,
        rank: skip + i + 1,
      }));
    ...
    const duelStatsMap = await computeDuelStatsBatch(
      this.prisma,
      allUsers.map((u) => u.id),
    );

    let processedUsers = allUsers.map((u) => {
      const ds = duelStatsMap.get(u.id) || {
        duelsWon: 0,
        duelsLost: 0,
        duelsTotal: 0,
        winRate: 0,
