...
describe('HintsService', () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        },
        {
          ...
          useValue: {
            findOne: jest.fn().mockResolvedValue({ id: 'chal_1', title: 'Two Sum', difficulty: 'easy', hints: ['hint1', 'hint2'] }),
          },
        },
        {
          provide: AiService,
          useValue: {
            generateProgressiveHint: jest.fn().mockResolvedValue('Gemini Hint Here'),
          },
        },
        {
  ...
  describe('recommendLevel', () => {
    ...
    it('should fallback to heuristic if intelligence engine fails', async () => {
      (intelligenceService.getSubmitDecision as jest.Mock).mockRejectedValueOnce(new Error('Network error'));
  ...
  describe('getHint', () => {
    ...
    it('should load level 5 if explicitly confirmed', async () => {
      ...
      (aiService.generateProgressiveHint as jest.Mock).mockResolvedValueOnce('Full Solution Here');
      ...
      expect((prismaService as any).hintInteraction.create).toHaveBeenCalled();
    });

    it('should serve static fallback if Gemini hint generation fails', async () => {
      (intelligenceService.getHintPolicy as jest.Mock).mockRejectedValueOnce(new Error('Down'));
