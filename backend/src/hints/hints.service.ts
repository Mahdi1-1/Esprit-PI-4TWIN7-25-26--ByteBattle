...
type HintStyle = 'concept' | 'strategy' | 'pseudocode' | 'partial_snippet' | 'near_solution';
...
export class HintsService {
   ...
   */
  async recommendLevel(userId: string, data: { challengeId: string, language: string, codeLength: number, wrongAnswerCount: number }) {
    ...
    try {
      const result = await this.intelligenceService.getSubmitDecision({
        ...
        cf_rating: this.mapDifficultyToRating((challenge as any).cfRating || (challenge as any).cf_rating || (challenge as any).difficulty),
        minutes_stuck: 0,
        attempts_count: data.wrongAnswerCount,
        last_hint_level: 0,
        challenge_tags: challenge.tags || [],
        code_lines: data.codeLength > 0 ? Math.max(1, Math.round(data.codeLength / 6)) : 40,
      });

      const rawLevel = Number(result?.recommended_hint_level ?? result?.needs_help ? result?.recommended_hint_level : 1);
      const level = Number.isFinite(rawLevel) ? Math.max(1, Math.min(5, rawLevel)) : 2;
      const fallbackPolicy = this.derivePolicyFromLevel(level, data.wrongAnswerCount);
      const mlHintStyle = this.parseHintStyle((result as any)?.hint_style);
      const mlHintIntensity = this.parseHintIntensity((result as any)?.hint_intensity);
      const mlHintTiming = this.parseHintTiming((result as any)?.hint_timing);
      const rawConfidence = Number(result?.hint_confidence ?? (result as any)?.confidence);
  ...
  async getHint(
    ...
    if (data.targetLevel === 5 && !data.confirmLevel5) {
      throw new HttpException('Le Niveau 5 nécessite une confirmation explicite.', HttpStatus.FORBIDDEN);
    }
    ...
    const effectiveStyle = data.hintStyle || this.derivePolicyFromLevel(data.targetLevel, data.attemptsCount).hintStyle;
    const effectiveIntensity = data.hintIntensity || this.derivePolicyFromLevel(data.targetLevel, data.attemptsCount).hintIntensity;
    const effectiveTiming = data.hintTiming || this.derivePolicyFromLevel(data.targetLevel, data.attemptsCount).hintTiming;
    ...
    try {
      ...
      const effectiveDecisionModel = (policy as any).decision?.model || data.decisionModel || 'intelligence_engine';
      const effectiveDecisionConfidence = (policy as any).decision?.confidence ?? data.decisionConfidence ?? null;
      ...
      await (this.prisma as any).hintInteraction.create({
        data: {
          userId,
          ...
          decisionConfidence: typeof effectiveDecisionConfidence === 'number' ? effectiveDecisionConfidence : null,
      ...
      return {
        ...
        decision: {
          ...
          confidence: typeof effectiveDecisionConfidence === 'number' ? effectiveDecisionConfidence : null,
          hintStyle: policyStyle,
          hintIntensity: policyIntensity,
          hintTiming: policyTiming,
        },
      ...
      this.logger.warn(`Fallback triggered for getting hint text: ${e.message}`);
    ...
    return {
      ...
      decision: {
        ...
        confidence: typeof data.decisionConfidence === 'number' ? data.decisionConfidence : null,
  ...
  private parseHintStyle(value: unknown): HintStyle | null {
    ...
    if (v === 'concept' || v === 'strategy' || v === 'pseudocode' || v === 'partial_snippet' || v === 'near_solution') {
  ...
  private derivePolicyFromLevel(level: number, wrongAnswerCount: number): {
    hintStyle: HintStyle;
    hintIntensity: HintIntensity;
    hintTiming: HintTiming;
  } {
    ...
    const hintIntensity: HintIntensity =
      safeLevel >= 4 || wrongAnswerCount >= 6 ? 'high' : safeLevel >= 2 ? 'medium' : 'low';

    const hintTiming: HintTiming = wrongAnswerCount >= 2 || safeLevel >= 3 ? 'now' : 'wait';
  ...
  private generateStaticFallbackHint(challenge: any, level: number): string {
    ...
    switch (level) {
      case 1:
        return hints.length > 0 ? `💡 Concept: ${hints[0]}` : "💡 Réfléchissez aux structures de données de base adaptées (ex: Maps ou Graphes).";
      case 2:
        return hints.length > 1 ? `🔍 Stratégie: ${hints[1]}` : "🔍 Divisez le problème en sous-fonctions.";
      case 3:
