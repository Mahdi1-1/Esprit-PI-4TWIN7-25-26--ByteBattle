...
import {
  ...
  CreateCanvasChallengeDto,
  GenerateChallengeDraftDto,
...
export class ChallengesController {
  ...
  @ApiOperation({
    summary:
      "Generate a challenge draft using AI (authenticated users, consumes 5 tokens)",
  })
  generateUserDraft(
    @CurrentUser("id") userId: string,
    @Body() dto: GenerateChallengeDraftDto,
  ) {
    return this.challengesService.generateDraftForUser(
      userId,
      dto.prompt,
      dto.kind || "CODE",
    );
  ...
  @ApiBearerAuth()
  @ApiOperation({
    summary: "Get challenge details (admin, includes hidden tests)",
  })
