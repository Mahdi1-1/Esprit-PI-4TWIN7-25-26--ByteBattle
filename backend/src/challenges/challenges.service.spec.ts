...
describe('ChallengesService', () => {
  ...
  describe('update()', () => {
    ...
    it('should map statementMd to descriptionMd in update', async () => {
      mockPrisma.challenge.findUnique.mockResolvedValue(baseChallenge);
      mockPrisma.challenge.update.mockResolvedValue({ ...baseChallenge, descriptionMd: 'updated' });
  ...
  describe('remove()', () => {
    ...
    it('should delete challenge and invalidate cache', async () => {
      ...
      expect(mockPrisma.challenge.delete).toHaveBeenCalledWith({ where: { id: 'chal-1' } });
      expect(mockCache.del).toHaveBeenCalledWith('challenge:public:chal-1');
      expect(mockCache.del).toHaveBeenCalledWith('challenge:admin:chal-1');
    });
  });
  ...
  describe('generateDraftForUser()', () => {
    ...
    it('should throw BadRequestException when non-premium user has insufficient tokens', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-1', isPremium: false, tokensLeft: 3 });
    ...
    it('should deduct 5 tokens for non-premium users', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-1', isPremium: false, tokensLeft: 10 });
      mockAiService.generateChallengeDraft.mockResolvedValue({ title: 'Sorting', tests: [] });
    ...
    it('should NOT deduct tokens for premium users', async () => {
      mockPrisma.user.findUnique.mockResolvedValue({ id: 'user-1', isPremium: true, tokensLeft: 0 });
      mockAiService.generateChallengeDraft.mockResolvedValue({ title: 'Sorting', tests: [] });
  ...
  describe('createCompanyChallenge()', () => {
    ...
    it('should throw BadRequestException when user has no recruiter/owner membership', async () => {
      ...
      await expect(service.createCompanyChallenge('user-1', dto)).rejects.toThrow(
        BadRequestException,
      );
    });

    it('should create challenge with draft status for employees_only visibility', async () => {
      mockPrisma.companyMembership.findFirst.mockResolvedValue({ id: 'm-1', role: 'recruiter' });
      mockPrisma.challenge.create.mockResolvedValue({ ...baseChallenge, status: 'draft' });
    ...
    it('should create challenge with published status for public visibility', async () => {
      mockPrisma.companyMembership.findFirst.mockResolvedValue({ id: 'm-1', role: 'owner' });
      mockPrisma.challenge.create.mockResolvedValue({ ...baseChallenge, status: 'published' });

      await service.createCompanyChallenge('user-1', { ...dto, visibility: 'public' });
