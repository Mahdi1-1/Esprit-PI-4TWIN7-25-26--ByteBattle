...
export class TeamsService {
    ...
    for (const team of teams) {
      ...
      const hackathonIds = registrations
        .filter((registration: any) =>
          (registration.members ?? []).some((member: any) =>
  ...
  private async toDto(
    ...
    const dto = await TeamResponseDto.fromPrisma(
      raw,
      requestingUserId,
  ...
  private async emitSystemNotification(
    userId: string,
    title: string,
    message: string,
    actionUrl = "/teams",
  ...
  private async getTeamOrThrow(teamId: string) {
    const team = await this.prisma.userTeam.findUnique({
      where: { id: teamId },
    });
    if (!team) {
  ...
  async getMyTeams(userId: string) {
    ...
    const registrationsByTeamId =
      await this.buildRegisteredHackathonIdsByTeamId(scopedTeams);
    return Promise.all(
      scopedTeams.map((team) =>
        this.toDto(team, userId, registrationsByTeamId[team.id] ?? []),
      ),
    );
  }
  ...
  async getAllTeams(userId: string) {
    ...
    const registrationsByTeamId =
      await this.buildRegisteredHackathonIdsByTeamId(teams);
    return Promise.all(
      teams.map((team) =>
        this.toDto(team, userId, registrationsByTeamId[team.id] ?? []),
      ),
  ...
  async getTeamById(teamId: string, userId: string) {
    ...
    const registrationsByTeamId =
      await this.buildRegisteredHackathonIdsByTeamId([team]);
  ...
  async inviteMember(
    ...
    if (members.length >= this.maxMembers) {
      throw new BadRequestException(
        `Team is full (max ${this.maxMembers} members)`,
      );
    }
  ...
  async leaveTeam(teamId: string, userId: string) {
    ...
    if (team.ownerId === userId) {
      if (members.length > 1) {
        throw new BadRequestException(
          "Delete the team or remove members before leaving",
        );
      }
  ...
  async deleteTeam(teamId: string, requesterId: string) {
    ...
    const memberIds = Array.from(
      new Set((team.members ?? []).map((member: any) => member.userId)),
    );
  ...
  async requestJoinByCode(userId: string, joinCode: string) {
    ...
    const team = await this.prisma.userTeam.findFirst({
  ...
  async approveJoinRequest(
    ...
    const request = requests.find(
      (item: any) => item.userId === targetUserId && item.status === "pending",
    );
    if (!request) {
    ...
    const updated = await this.prisma.userTeam.update({
      ...
      data: {
        ...
        joinRequests: requests.filter(
          (item: any) => item.userId !== targetUserId,
        ),
      },
    });
    ...
    const registrationsByTeamId =
      await this.buildRegisteredHackathonIdsByTeamId([updated]);
    return this.toDto(
      updated,
      requesterId,
      registrationsByTeamId[updated.id] ?? [],
    );
  }
  ...
  async rejectJoinRequest(
    ...
    await this.setJoinRequests(
      teamId,
      (team.joinRequests ?? []).filter(
        (request: any) => request.userId !== targetUserId,
      ),
    );
  ...
  async registerToHackathon(
    ...
    if (team.ownerId !== requesterId) {
      throw new BadRequestException(
        "Only the team captain can register the team to a hackathon",
      );
    }

    if (
      !(team.members ?? []).some((member: any) => member.userId === requesterId)
    ) {
      throw new BadRequestException("You are not a member of this team");
    }
    ...
    const conflictingRegistration = hackathonRegistrations.find(
      (registration: any) =>
        (registration.members ?? []).some((member: any) =>
          teamMemberIds.has(member.userId),
        ),
    );

    if (conflictingRegistration) {
      throw new BadRequestException(
        "At least one team member is already registered in this hackathon",
      );
    }
