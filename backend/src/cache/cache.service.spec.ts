...
describe('CacheService', () => {
  const createConfigService = (values: Record<string, any> = {}) =>
    ({
      get: jest.fn((key: string, defaultValue?: any) => (key in values ? values[key] : defaultValue)),
  ...
  it('does nothing when Redis is disabled', async () => {
    ...
    await expect(service.set('key', { hello: 'world' })).resolves.toBeUndefined();
  ...
  it('initializes Redis with configured connection values', async () => {
    ...
    await expect(service.get<{ value: number }>('cache:key')).resolves.toEqual({ value: 1 });
    await expect(service.set('cache:key', { value: 2 }, 30)).resolves.toBeUndefined();
    await expect(service.set('cache:key', { value: 3 })).resolves.toBeUndefined();
    await expect(service.del('cache:key')).resolves.toBeUndefined();

    expect(redisInstance.get).toHaveBeenCalledWith('cache:key');
    expect(redisInstance.setex).toHaveBeenCalledWith('cache:key', 30, JSON.stringify({ value: 2 }));
    expect(redisInstance.set).toHaveBeenCalledWith('cache:key', JSON.stringify({ value: 3 }));
  ...
  it('deletes keys by pattern using scanStream and pipeline', async () => {
    const dataHandlerCallbacks: Array<(keys: string[]) => Promise<void> | void> = [];
    const pipeline = {
      del: jest.fn(),
      exec: jest.fn().mockResolvedValue([]),
    };
    const scanStream = {
      on: jest.fn((event: string, handler: (keys: string[]) => Promise<void> | void) => {
        if (event === 'data') {
    ...
    expect(redisInstance.scanStream).toHaveBeenCalledWith({ match: 'leaderboard:*', count: 100 });
