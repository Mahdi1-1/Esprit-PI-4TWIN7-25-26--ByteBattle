import {
  Controller, Get, Patch, Delete, Put, Post,
...
export class NotificationsController {
  ...
  @Get('preferences')
  @ApiOperation({ summary: 'Get current user notification preferences (returns defaults if not set)' })
  ...
  @Put('preferences')
  @ApiOperation({ summary: 'Create or update notification preferences (upsert)' })
  @ApiBody({ type: UpdateNotificationPreferenceDto })
  @ApiResponse({ status: 200, description: 'Updated NotificationPreference' })
  updatePreferences(@Request() req, @Body() dto: UpdateNotificationPreferenceDto) {
