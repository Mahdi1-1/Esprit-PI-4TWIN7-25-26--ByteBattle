...
describe("NotificationsGateway", () => {
  ...
  describe("handleConnection()", () => {
    ...
    it("should disconnect when token is invalid", async () => {
      const client = makeSocket({
        handshake: { auth: { token: "Bearer bad-jwt" }, headers: {} },
      });
  ...
  describe("emitToUser()", () => {
    it("should emit to all sockets of a user", async () => {
      ...
      expect(toMock().emit).toHaveBeenCalledWith("new_notification", {
        id: "notif-1",
      });
    });
  ...
  describe("emitBroadcast()", () => {
    it("should emit to all connected clients", () => {
      ...
      gateway.emitBroadcast("system_announcement", {
        message: "Maintenance in 5 min",
      });

      expect(emitSpy).toHaveBeenCalledWith("system_announcement", {
        message: "Maintenance in 5 min",
      });
    });
  });
