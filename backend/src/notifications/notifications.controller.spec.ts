...
describe("NotificationsController", () => {
  ...
  describe("bulkMarkRead", () => {
    it("should bulk mark specified notifications as read", async () => {
      ...
      expect(mockNotificationsService.bulkMarkRead).toHaveBeenCalledWith(
        ["n-1", "n-2"],
        "user-1",
      );
  ...
  describe("bulkArchive", () => {
    it("should bulk archive specified notifications", async () => {
      ...
      expect(mockNotificationsService.bulkArchive).toHaveBeenCalledWith(
        ["n-1", "n-2"],
        "user-1",
      );
      expect(result).toEqual({ count: 2 });
  ...
  describe("archive", () => {
    it("should archive a single notification", async () => {
      mockNotificationsService.archive.mockResolvedValue({
        id: "n-1",
        isArchived: true,
      });
      ...
      expect(mockNotificationsService.archive).toHaveBeenCalledWith(
        "n-1",
        "user-1",
      );
