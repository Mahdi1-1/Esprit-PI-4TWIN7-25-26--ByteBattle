...
describe('NotificationEmitterService', () => {
  ...
  beforeEach(async () => {
    ...
    service = module.get<NotificationEmitterService>(NotificationEmitterService);
    jest.clearAllMocks();
  });
  ...
  describe('emit()', () => {
    it('should create a Notification document in DB and push via WebSocket', async () => {
      ...
      const createdNotif = { id: 'notif-1', ...baseDto, recipientId: 'user-1', isRead: false, isArchived: false, readAt: null, createdAt: new Date() };
      mockPrisma.notification.create.mockResolvedValue(createdNotif);

      await service.emit(baseDto);

      expect(mockPrisma.notification.create).toHaveBeenCalledTimes(1);
      expect(mockGateway.emitToUser).toHaveBeenCalledWith('user-1', 'notification:new', expect.objectContaining({ type: 'discussion_reply' }));
    });
    ...
    it('should skip when category is disabled in preferences (non-critical)', async () => {
      mockPreferenceService.getOrDefault.mockResolvedValue({ ...DEFAULT_PREFS, discussion: false });
    ...
    it('should persist but NOT push during quiet hours (non-critical)', async () => {
      ...
      mockPreferenceService.getOrDefault.mockResolvedValue({ ...DEFAULT_PREFS, quietStart: '00:00', quietEnd: '23:59' });
      mockPrisma.notification.findFirst.mockResolvedValue(null);
      const createdNotif = { id: 'notif-1', ...baseDto, recipientId: 'user-1', isRead: false, isArchived: false, readAt: null, createdAt: new Date() };
      mockPrisma.notification.create.mockResolvedValue(createdNotif);
  ...
  describe('emitBroadcast()', () => {
    it('should create one notification per active user and emit broadcast WebSocket', async () => {
      ...
      expect(mockPrisma.notification.createMany).toHaveBeenCalledWith(
        expect.objectContaining({ data: expect.arrayContaining([expect.objectContaining({ recipientId: 'u1' })]) })
      );
      expect(mockGateway.emitBroadcast).toHaveBeenCalledWith('notification:new', expect.any(Object));
    });

    it('should do nothing when no active users', async () => {
      mockPrisma.user.findMany.mockResolvedValue([]);
      await service.emitBroadcast({ type: 'system_announcement', category: 'system', priority: 'high', title: 'X', message: 'Y' });
